...
/**
 * 嵌套滑动 外部 RecyclerView，扩展RecyclerView 让他支持内部嵌套滑动<br/>
 * @author 林佐跃 <br/>
 */
class ParentRecyclerView : RecyclerView, NestedScrollingParent3, NestedScrollingParent2 {

    //RecyclerView中有一个 mRecycler 和 mState final 的 全局变量，
    //所以这里会通过java反射来持有 mRecycler 和 mState的引用
    //嵌套滑动需要用到这两个类
    private var mTempRecycler: Recycler? = null

    private var mTempState: State? = null
    ...
    private fun getMState(): State {
        if (mTempState == null) {
            val field = RecyclerView::class.java.getDeclaredField("mState")
            field.isAccessible = true
            mTempState = field.get(this) as State?
        }
        return mTempState!!
    }

    private fun getMRecycler(): Recycler {
        if (mTempRecycler == null) {
            val field = RecyclerView::class.java.getDeclaredField("mRecycler")
            field.isAccessible = true
            mTempRecycler = field.get(this) as Recycler?
        }
        return mTempRecycler!!
    }
